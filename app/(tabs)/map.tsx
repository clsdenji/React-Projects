...
import MapView, { Marker, Region, LatLng, UrlTile} from "react-native-maps";
...
const getEstimatedMinutes = (m: number, speedMps = 5) => Math.ceil(m / speedMps / 60);

/** ---------- Overpass (OSM) ---------- */
async function fetchParkingsOverpass(
  lat: number,
  lon: number,
  radius = 1200
): Promise<ParkingSpot[]> {
  ...
  const res = await fetch("https://overpass-api.de/api/interpreter", {
    ...
    headers: {
      ...
      "User-Agent": "SparkParking/1.0 (contact: your-email@domain.com)",
    },
  ...
  const raw: Array<ParkingSpot | null> = (json.elements || []).map((el: any, idx: number) => {
  ...
  // TS-safe type guard + sort
  const items = raw.filter((x): x is ParkingSpot => x !== null);
...
async function geocodePlace(
  ...
  const res = await fetch(url, {
    headers: { "User-Agent": "SparkParking/1.0 (contact: your-email@domain.com)" },
  });
...
const MapScreen = () => {
  ...
  const filtered = useMemo(
    () => parkingSpots.filter((p) => p.name.toLowerCase().includes(search.toLowerCase())),
    [parkingSpots, search]
  );
  ...
  return (
    <View style={styles.container}>
      <MapView
        ...
        region={region}
        showsUserLocation
        mapType="none" // hide Google basemap so only OSM tiles are shown
      ...
      >        
        ...
        {filtered.map((spot) => (
          <Marker
            ...
            description={`~${Math.round(spot.distance)} m • ${getEstimatedMinutes(
              spot.distance
            )} min walk`}
            pinColor="green"
            onPress={() => zoomTo({ latitude: spot.latitude, longitude: spot.longitude })}
          />
        ))}
      </MapView>
        ...
        <FlatList
          ...
          renderItem={({ item }) => (
            <TouchableOpacity
              ...
              onPress={() => zoomTo({ latitude: item.latitude, longitude: item.longitude })}
            >
