...
export default function LoginPage() {
    ...
    (async () => {
      ...
      try {
        ...
        if (status !== "granted") {
          ...
          Alert.alert("Location Permission", "You need to enable location permission to continue.");
  ...
  const validateEmail = (v: string) => {
    ...
    setEmailError(emailRegex.test(v.trim()) ? "" : "Please enter a valid email address.");
  };
  ...
  const handleSubmit = async () => {
    ...
    if (isLogin) {
      ...
      if (error) {
        ...
        router.replace("/(tabs)/map"); // adjust to your route
      }
    } else {
  ...
  if (locationError) {
    return (
      <View style={styles.errorContainer}>
        <Text style={{ color: AMBER, marginBottom: 20 }}>{locationError}</Text>
  ...
  return (
    <SafeAreaView style={styles.background}>
      ...
      >
        ...
        >
          {/* Scroll so small screens don't clip content */}
          <ScrollView
